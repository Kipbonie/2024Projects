    ...
    */
    ...
    if ($post) {
        if(
            ...
            $unamematch = preg_match('/^[A-Za-z][A-Za-z0-9_]{3,}$/', $_POST['uname']);
            $fnamematch = preg_match('/^[A-Za-z]+$/', $_POST['fname']);
            $lnamematch = preg_match('/^[A-Za-z]+$/', $_POST['lname']);
            $emailmatch = preg_match('/^[A-Za-z_0-9]+@[A-Za-z]+.[A-Za-z]+$/', $_POST['email']);
            $passmatch = preg_match('/.{5,}/',$_POST['pass']);
            $peq = $_POST['pass']==$_POST['repass'];
            if($unamematch&&$fnamematch&&$lnamematch&&$emailmatch&&$passmatch&&$peq) {
                try {
...
<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
    ...
    if($post&&!$empty_fields&&!$unamematch) echo '<span>Username can contain alphabet letters, numbers and underscore(_), but must begin with a letter. It must be at least 4 character long.<br></span>';
    ...
    if($post&&!$empty_fields&&!($lnamematch&&$fnmatch)) echo '<span>Name can only contain alphabet letters.<br></span>';
    ...
    if($post&&!$empty_fields&&!$emailmatch) echo '<span>Email must be of format example@site.domain<br></span>';
    ...
    if($post&&!$empty_fields&&!$passmatch) echo '<span>Password must be at least 5 character long</span>';
    if($post&&!$empty_fields&&$passmatch&&!$peq) echo '<span>Password don\'t match</span><br>';
